...
def get_attr_qq(driver, name, url, index_num, names, urls):
    ...
    for i in range(4):
        ...
        actions.perform()
        data['skill_' + str(i)] = driver.find_element_by_xpath(
            '/html/body/div[3]/div[2]/div/div[1]/div[2]/div/div/div[' + str(i + 1) + ']/p[1]/b').text
        tmp = driver.find_element_by_xpath(
            '/html/body/div[3]/div[2]/div/div[1]/div[2]/div/div/div[' + str(i + 1) + ']/p[1]/span[1]').text
        data['skill_' + str(i) + '_cooling'] = pattern.search(tmp).group()
        tmp = driver.find_element_by_xpath(
            '/html/body/div[3]/div[2]/div/div[1]/div[2]/div/div/div[' + str(i + 1) + ']/p[1]/span[2]').text
        data['skill_' + str(i) + '_cost'] = pattern.search(tmp).group()
        data['skill_' + str(i) + '_desc'] = driver.find_element_by_xpath(
            '/html/body/div[3]/div[2]/div/div[1]/div[2]/div/div/div[' + str(i + 1) + ']/p[2]').text
...
def get_attr_17173(driver, name, url, index_num, names, urls):
    ...
    nodes = driver.find_elements_by_xpath(
        '/html/body/div[4]/div[2]/div[2]/div[1]/div[2]/div[2]/div[2]/div/div[3]/div/div[2]/ul/li')
...
def get_attr_4399(driver, name, url, index_num, names, urls):
    ...
    data['item'] = pattern.search(
        driver.find_element_by_xpath('/html/body/div[4]/div[1]/div[1]/div[3]/p[4]').text).group().replace(' ', '')
    data['item_type'] = pattern.search(
        driver.find_element_by_xpath('/html/body/div[4]/div[1]/div[1]/div[3]/p[5]').text).group().replace(' ', '')
    data['item_price'] = pattern.search(
        driver.find_element_by_xpath('/html/body/div[4]/div[1]/div[1]/div[3]/p[6]').text).group().replace(' ', '')
    data['item_attr'] = pattern.search(
        driver.find_element_by_xpath('/html/body/div[4]/div[1]/div[1]/div[3]/p[7]').text).group().replace(' ', '')
    data['item_passive'] = pattern.search(
        driver.find_element_by_xpath('/html/body/div[4]/div[1]/div[1]/div[3]/p[8]').text).group().replace(' ', '')
...
if __name__ == "__main__":
    ...
    # 获取qq上的英雄列表
    hero_data1 = get_url(driver, 'http://pvp.qq.com/web201605/herolist.shtml',
    ...
    # 获取4399上的装备列表
    item_data0 = get_url(driver, "http://news.4399.com/gonglue/wzlm/daoju/", '//*[@id="hreo_list"]/li/a', action=True,
                         action_node='//*[@id="hero_more"]/a')
    ...
    hero_data.to_csv(r'F:\ericstudy\wangzherongyao\newVersion\data\hero.csv')
    item_data.to_csv(r'F:\ericstudy\wangzherongyao\newVersion\data\item.csv')
